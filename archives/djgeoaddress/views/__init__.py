...
@require_GET
def _format_address_result(addr: Dict[str, Any]) -> Dict[str, Any]:
    """Format a single address result with id and admin_url."""
    ...
    formatted_result = {k: v for k, v in addr.items() if k != "_raw_response"}
...
def _address_autocomplete_view_impl(request):
    ...
    if not view_enabled:
        return JsonResponse({"error": "Address autocomplete view is disabled"}, status=403)
    ...
    try:
        ...
        formatted_results = [_format_address_result(addr) for addr in results]
