...
class AddressAutocompleteWithEditWidget(forms.MultiWidget):
    ...
    def __init__(self, attrs=None):
        ...
        address_widgets = [
            ...
            forms.TextInput(
                attrs={"placeholder": _("Postal code"), "class": "address-postal-code"}
            ),
            ...
            forms.TextInput(
                attrs={"placeholder": _("Country (ISO code)"), "class": "address-country"}
            ),
            forms.TextInput(
                attrs={"placeholder": _("Municipality"), "class": "address-municipality"}
            ),
    ...
    def decompress(self, value):
        ...
        if isinstance(value, dict):
            ...
            formatted_address = ", ".join(
                filter(None, address_parts + [" ".join(filter(None, city_parts))] + state_country)
            )
            municipality = (
                value.get("municipality") or value.get("extras", {}).get("municipality") or ""
            )
            confidence = value.get("confidence")
            relevance = value.get("relevance")
            backend_used = value.get("backend_used") or value.get("backend") or ""
            backend_reference = (
                value.get("backend_reference") or value.get("address_reference") or ""
            )
            return [
                formatted_address,
                value.get("line1") or value.get("address_line1") or "",
    ...
    def value_from_datadict(self, data, files, name):
        ...
        if (
            autocomplete_value
            and isinstance(autocomplete_value, dict)
            and any(autocomplete_value.values())
        ):
    ...
    def get_context(self, name, value, attrs):
        ...
        if (
            autocomplete_value
            and isinstance(autocomplete_value, str)
            and autocomplete_value.strip()
        ):
            context["autocomplete_widget_html"] = self._autocomplete_widget.render(
                name, autocomplete_value, attrs
            )
        else:
            context["autocomplete_widget_html"] = self._autocomplete_widget.render(
                name, None, attrs
